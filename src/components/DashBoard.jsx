...
const Item = ({ video, index, MoveVideo }) => {
  const [, ref] = useDrag({
    type: "VIDEO",
  });
  ...
  return (
    <div ref={(node) => ref(drop(node))}>
      <div className="flex justify-between border border-[#FFFFFF1F] items-center pl-4 mb-4 h-24 rounded-[16px] cursor-pointer">
        <td className="flex mr-8">{index < 9 ? `0${index + 1}` : index + 1}</td>
